...
- **`fs.watch` hot-reload not implemented (low)** — Design §3 (file-write contract) and §6 (`CatalogStore.startWatch`/`stopWatch`) specify hot-reloading the catalog when `catalog.json` changes on disk. The current `CatalogStore` exposes no watcher; the server reads the catalog only at startup. NFR15 ("Adding an article needs no downtime") is therefore partially met: a server restart is currently required after each publish. Recommended fix: implement `startWatch()`/`stopWatch()` per design §6.
